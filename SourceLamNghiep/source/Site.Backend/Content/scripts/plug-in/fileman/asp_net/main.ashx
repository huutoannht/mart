...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        try{
            ...
            if (action == "UPLOAD"){
                ...
                _r.Write("parent.fileUploaded(" + GetErrorRes(sb.ToString()) + ");");
                _r.Write("</script>");
            }
            else{
    ...
    protected string GetResultStr(string type, string msg, string fileName = null)
    {
        return "{\"res\":\"" + type + "\",\"msg\":\"" + msg.Replace("\"", "\\\"") + "\", fileName:\"" + (!string.IsNullOrWhiteSpace(fileName) ? fileName : string.Empty) + "\"}";
    }
    protected string GetSuccessRes(string msg, string fileName = null)
    {
        return GetResultStr("ok", msg, fileName);
    }
            ...
            for(int i = 0; i < HttpContext.Current.Request.Files.Count; i++){
                if (CanHandleFile(HttpContext.Current.Request.Files[i].FileName))
                {
                    ...
                    if(GetFileType(new FileInfo(filename).Extension) == "image"){
                        ...
                        res = GetSuccessRes(string.Empty, filename);
                    }
                }
                else
